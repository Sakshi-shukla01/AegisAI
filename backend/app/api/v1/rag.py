...
class RAGQueryResponse(BaseModel):
    ...
    groundedness_score: float = Field(
        0.0,
        description=(
            "Cosine similarity score (0.0 to 1.0) measuring answer "
            "groundedness in retrieved chunks."
        ),
    )
    low_confidence: bool = Field(
        False,
        description="True if groundedness score falls below the accepted threshold.",
    )
